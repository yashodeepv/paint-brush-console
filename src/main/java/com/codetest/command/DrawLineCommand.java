...
public class DrawLineCommand implements ConsoleCommand {

    @Override
    public void execute(List<String> params, DrawingCanvas drawingCanvas) {
        if(params.size() != 4) {
            throw new IllegalArgumentException("Invalid parameters. Usage : l x1 y1 x2 y2");
        }
        ...
        drawingCanvas.drawLine(x1, y1, x2, y2);
        drawingCanvas.show();
    }
}
